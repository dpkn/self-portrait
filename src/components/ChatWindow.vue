<template>
  <div id="phone">
    <header>
      ...
      <span>Dani&euml;l 2 ({{this.inactivityTimer}})</span>
    </header>
    <section id="scroll">
...
export default {
  ...
  mounted(){
   ...
   // Since this work is shown at an exhibition,
   // reset the state of the app when people aren't interacting with it for a minute
   window.setInterval(()=>{
     this.inactivityTimer++;
     if(this.inactivityTimer>=this.inactivtyLimit){
       this.inactivityTimer = 0;
       document.querySelector('#message_receive').play()
       this.messages.push({user_id: 2, content: 'Hallo waarom praat niemand tegen mij.'});

       setTimeout(() => {
         this.speak('Hallo waarom praat niemand tegen mij.',0);
       }, 500);
       //this.reset()
     }
   }, 1000);
  ...
  if (speechSynthesis.onvoiceschanged !== undefined) {
    speechSynthesis.onvoiceschanged = () =>{
      this.synthVoices = this.speachSynth.getVoices();
      console.log(this.synthVoices)
    }
  }

  // Create the LSTM Generator passing it the model directory
  this.lstm = ml5.LSTMGenerator('models/input/', (e)=>{
    console.log('model readye',e)
  });
  ...
  data() {
    return{
      ...
      inactivityTimer:0,
      inactivtyLimit:20000
    }
  },
  methods: {
    // This function uses the SpeechSynthesis API to read the replies out loud
    speak(text,voice){
          ...
          axios.get('http://127.0.0.1:5000/message')
            .then(function (response) {
              ...
              setTimeout(function(){
                ...
                setTimeout(function(){
                  document.querySelector('#message_receive').play()
                  that.messages.push({user_id: 2, content: response.data.message});

                  setTimeout(() => {
                    that.speak(response.data.message,0);
                  }, 500);

                  that.state = NOT_TYPING
                  that.placeholder = 'Type something and press ENTER to send'
                  that.inactivityTimer = 0;
                }, 2000);
              }, 400);
